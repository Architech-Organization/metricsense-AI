...
class EngineeringRouter:
    ...
    async def engineering_summary(self, project: str = Query(...), priority: str = Query(...), month: str = Query(...)):
        try:
            data = await self.board_issue_service.read_board_issues(project, priority, month)
            summary_service = self.summary_factory.create_summary(self.summary_factory.service_name)
            summary = summary_service.generate_summary_for_critical_defects_identified(data)
